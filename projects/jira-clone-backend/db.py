...
# Users
async def db_create_user(user: UsersModel) -> bool:
    CUsers.insert_one(user.to_dict())


async def db_update_user(user_id: str, user: Dict[str, Any]) -> bool:
    res = CUsers.update_one({"id": user_id}, {"$set": user})
    return res.modified_count > 0


async def db_list_users() -> List[UsersModel]:
    return [UsersModel.from_dict(r) for r in CUsers.find()]


async def db_list_user_tasks(user_id: str) -> List[TasksModel]:
    return [TasksModel.from_dict(r) for r in CTasks.aggregate( [ { "$match": { "assignee": user_id } } ] )]


async def db_retrieve_user(user_id: str) -> UsersModel:
...
# Tasks
async def db_create_task(task: TasksModel) -> bool:
    CTasks.insert_one(task.to_dict())


async def db_update_task(task_id: str, task: Dict[str, Any]) -> bool:
    res = CTasks.update_one({"id": task_id}, {"$set": task})
    return res.modified_count > 0


async def db_list_tasks() -> List[TasksModel]:
    return [TasksModel.from_dict(r) for r in CTasks.find()]


async def db_list_task_comments(task_id: str) -> List[CommentsModel]:
    return [CommentsModel.from_dict(r) for r in CComments.aggregate( [ { "$match": { "task": task_id } } ] )]


async def db_retrieve_task(task_id: str) -> TasksModel:
...
# Comments
async def db_create_comment(comment: CommentsModel) -> bool:
    CComments.insert_one(comment.to_dict())


async def db_update_comment(comment_id: str, comment: Dict[str, Any]) -> bool:
    res = CComments.update_one({"id": comment_id}, {"$set": comment})
    return res.modified_count > 0


async def db_list_comments() -> List[CommentsModel]:
    return [CommentsModel.from_dict(r) for r in CComments.find()]


async def db_retrieve_comment(comment_id: str) -> CommentsModel:
